...
# copied from: http://www.cs.mcgill.ca/~abatko/computers/programming/perl/howto/getopts
{
	...
	my $opt_string = 'hvlrzdsftnp:ux:g';
...
$0 aifile.ai | xargs -I {} ln -vs
	...
	-l make absolute symlink in current directory if file linked from Illustrator exists elsewhere
	-r make relative symlink in current directory if file linked from Illustrator exists elsewhere
	...
	-g git status of src and target
...
sub mysymlink{
	...
	# if we want relative symlinks
	$src=File::Spec->abs2rel($src,$targetdir) if $opt{r};
	
	if(-e $src){
		my $opts=$opt{f}?"-fsv":"-sv";		
		my $cmd="cd \"$targetdir\" && ln $opts \"$src\" \"$target\"";			
		myexec("$cmd");
	} else {
...
sub mygitstatus{
	...
	return "--" unless (-f $src);
	my $wd = dirname($src);
	$src=File::Spec->canonpath($src,$targetdir);
	my $gitstatus =  `cd \"$wd\" && git status --porcelain -u no -- \"$src\" `;
	$gitstatus=~s/^(.{2}).*\n/$1/;
	return $gitstatus;
}
...
# not sure why we need two % signs here
		if ( $lines[$i]=~/^DocumentFiles\:(.*?)\W+%/){
			print mygitstatus($1,$InDir)," " if $opt{g};
			print mystatus($1,$InDir)," " if $opt{u} and not $opt{n};
			print "\"$1\" ",$opt{d}?$ProtectedInDir:"","\n" unless $opt{n};
			$i++;
			mysymlink($1,$InDir) if $opt{l} or $opt{r};
			while($lines[$i]=~/^[+](.*?)\W\%.*$/){
#				print "\"$1\" $InDir\n"; $i++;			
				print mygitstatus($1,$InDir)," " if $opt{g} and not $opt{n};
				print mystatus($1,$InDir)," " if $opt{u} and not $opt{n};
				print "\"$1\" ",$opt{d}?$ProtectedInDir:"","\n"unless $opt{n};
				$i++;
				mysymlink($1,$InDir) if $opt{l} or $opt{r};					
